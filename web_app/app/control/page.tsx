...
export default function ControlPage(){
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      ...
      <section className={styles.clientFilters}><button className={clientFilter==="all"?styles.selected:""} onClick={()=>setClientFilter("all")}>Все клиенты <b>{data?.items.length||0}</b></button>{clients.map(client=><button key={client} className={clientFilter===client?styles.selected:""} onClick={()=>setClientFilter(client)} title={client}>{client}<b>{data?.items.filter(item=>item.client===client).length||0}</b></button>)}</section>

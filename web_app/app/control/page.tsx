...
import {listCertificates,signDetached} from "./cades";
...
export default function ControlPage(){
  ...
  const revokeForwarding=async(item:ControlItem)=>{const reason=window.prompt("Причина отзыва поручения");if(!reason)return;const terminationDate=window.prompt("Дата прекращения исполнения (ГГГГ-ММ-ДД)",new Date().toISOString().slice(0,10));if(!terminationDate)return;const actualExpenses=window.prompt("Фактические расходы экспедитора, руб. Оставьте пустым, если расходов нет","")??"";const cargoInstructions=window.prompt("Указания по грузу (необязательно)","")??"";const signer=window.prompt("ФИО подписанта полностью",data?.user?.name||"");if(!signer)return;const [signerLastName,signerFirstName,...middle]=signer.trim().split(/\s+/);if(!signerLastName||!signerFirstName){window.alert("Укажите минимум фамилию и имя");return;}try{const response=await fetch("/api/kontur/forwarding-revoke",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messageId:item.messageId,entityId:item.entityId,sourceMessageId:item.sourceMessageId,sourceEntityId:item.sourceEntityId,reason,terminationDate,actualExpenses,cargoInstructions,signerLastName,signerFirstName,signerMiddleName:middle.join(" ")})});if(!response.ok){const result=await response.json();throw new Error(result.error||"Не удалось сформировать Т3");}const bytes=new Uint8Array(await response.arrayBuffer());let binary="";for(const byte of bytes)binary+=String.fromCharCode(byte);const contentBase64=btoa(binary);const certificates=await listCertificates();if(!certificates.length)throw new Error("В хранилище пользователя нет доступных сертификатов");const list=certificates.map((certificate,index)=>`${index+1}. ${certificate.subject}\n   действует до ${certificate.validTo}`).join("\n");const selected=Number(window.prompt(`Выберите сертификат для подписи:\n${list}`,"1"));if(!Number.isInteger(selected)||selected<1||selected>certificates.length)return;if(!window.confirm(`Подписать и отправить отзыв поручения №${item.number} в Диадок? После отправки отменить действие нельзя.`))return;const signatureBase64=await signDetached(contentBase64,certificates[selected-1].thumbprint);const sent=await fetch("/api/kontur/forwarding-revoke/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messageId:item.messageId,parentEntityId:item.sourceEntityId||item.entityId,contentBase64,signatureBase64})});const result=await sent.json();if(!sent.ok)throw new Error(result.error||"Не удалось отправить Т3");window.alert("Отзыв подписан и отправлен в Диадок");await load(true);}catch(value){window.alert(value instanceof Error?value.message:"Не удалось подписать и отправить Т3");}};
  useEffect(()=>{void load();},[]);

  ...
  return <main className={styles.shell}>
    <header className={styles.topbar}><div className={styles.logo}>А</div><div><strong>Создание ЭПД</strong><span>версия {appPackage.version}</span></div><nav><a className={styles.activeTab} href="/workspace">Создание документов</a><a href="/forwarding-orders">Поручения клиентам</a><a href="/control">Контроль подписания</a><a href="/statistics">Статистика</a><a href="/edo-settings">ID ЭДО</a></nav><i/><details className={styles.headerService}><summary><div className={styles.serviceMeters}><span><b>TMS</b><progress value={tmsProgressValue} max={100}/><em>{tmsBusy||restoreProgress.active?tmsProgressValue+"%":ready?"готово":"нет данных"}</em></span><span><b>Контур</b>{konturChecking?<progress/>:<progress value={konturProgressValue} max={100}/>}<em>{konturChecking?"проверка":edoBusy?edoProgress+"%":kontur.connected?"подключён":"не подключён"}</em></span></div><b>Настройки</b></summary><section className={styles.serviceDrawer}>
      <section className={styles.konturPanel}>
        ...
        {!kontur.connected?<button disabled={!kontur.configured} onClick={()=>{window.location.href="/api/kontur/login";}}>{kontur.configured?"Войти через Контур":"Нет настроек API"}</button>:<div><span className={styles.konturActions}><button disabled={edoBusy} onClick={syncEdoFromKontur}>{edoBusy?`Обновляем · ${edoProgress}%`:"Обновить контрагентов"}</button><button className={styles.secondaryButton} onClick={()=>{window.location.href="/api/kontur/login";}}>Переподключить</button></span><small>{edoSource?.updatedAt?`Контрагенты обновлены ${new Date(edoSource.updatedAt).toLocaleString("ru-RU")}`:"Контрагенты ещё не обновлялись"}</small>{edoBusy&&<progress value={edoProgress} max={100}/>}</div>}
      </section>
      <section className={styles.tmsPanel}>
        <div className={styles.tmsBar}><div><strong>Данные TMS</strong><small>{updatedLabel(tmsUpdatedAt)}</small><small>Грузы, ТТН/CMR и точки маршрута</small></div><button disabled={tmsBusy || busy} onClick={updateFromTms}>{tmsBusy ? "Обновляем…" : ready ? "Обновить данные" : "Загрузить данные из TMS"}</button></div>
    ...
    <section className={styles.dynamicStatus} aria-live="polite"><i/>{tmsBusy||restoreProgress.active?"Скачиваю файлы с сервера, подождите…":edoBusy?"Обновляю данные из Контур.Диадок, подождите…":konturChecking?"Подключаю интеграцию с Контур, подождите…":ready?updatedLabel(tmsUpdatedAt):"Загрузите данные TMS для начала работы"}</section>

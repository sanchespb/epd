...
async function loadDocumentStatistics(){
  const cacheKey="etrn-title-status-from-2026-09";const cached=statisticsCache.get(cacheKey);if(cached?.expiresAt>Date.now())return cached.value;
  ...
  const items=await mapLimit(unique,8,async document=>{const flow=flowById.get(`${document.MessageId}:${document.EntityId}`);if(flow?.DocumentInfo?.IsDeleted)return null;let xml="";try{xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,document.EntityId);}catch{}const firstParty=(...tags)=>tags.map(tag=>xmlParty(xml,tag)).find(value=>value!=="Не указано")||"Не указано";const client=firstParty("СвЗак","СвГО");const carrier=firstParty("СвПер","СвИсп");const titles=flow?.Docflow?.Titles||[];const titleSigned=titleIndex=>titles.some(title=>Number(title.TitleIndex)===titleIndex&&title.AuthorSigning?.IsFinished===true);const rawDate=document.DocumentDate||document.CreationTimestamp||"";const match=String(rawDate).match(/^(\d{2})\.(\d{2})\.(\d{4})/);const parsed=match?new Date(`${match[3]}-${match[2]}-${match[1]}T00:00:00+03:00`):new Date(rawDate);const month=Number.isNaN(parsed.getTime())?"Не определён":`${parsed.getFullYear()}-${String(parsed.getMonth()+1).padStart(2,"0")}`;return {month,client,carrier,carrierTitle2Signed:titleSigned(1),clientTitle3Signed:titleSigned(2)};});

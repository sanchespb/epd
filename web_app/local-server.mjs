...
async function loadDocumentStatistics(){
  const cacheKey="from-2026-09";const cached=statisticsCache.get(cacheKey);if(cached?.expiresAt>Date.now())return cached.value;
  const config=konturConfig(),accessToken=await getKonturAccessToken(),from=new Date(2026,8,1);
  const formatDate=date=>`${String(date.getDate()).padStart(2,"0")}.${String(date.getMonth()+1).padStart(2,"0")}.${date.getFullYear()}`;
  const documents=[];let afterIndexKey="";for(let page=0;page<20;page++){const body={DocumentTypeNamedIds:["LogisticsWaybill","LogisticsOrderRequest","LogisticsForwardingOrder"],DocumentCategory:"Any",FromDocumentDate:formatDate(from),SortDirection:"Descending",Count:100,...(afterIndexKey?{AfterIndexKey:afterIndexKey}:{})};const list=await diadocJson(`/V4/GetDocuments?boxId=${encodeURIComponent(config.boxId)}`,accessToken,{method:"POST",body:JSON.stringify(body)});const portion=list.Documents||[];documents.push(...portion);if(portion.length<100)break;afterIndexKey=portion.at(-1)?.IndexKey||"";if(!afterIndexKey)break;}
  const unique=[];const seen=new Set();for(const document of documents){const key=`${document.MessageId}:${document.EntityId}`;if(!document.IsDeleted&&!seen.has(key)){seen.add(key);unique.push(document);}}
  ...
  const items=await mapLimit(unique,8,async document=>{const flow=flowById.get(`${document.MessageId}:${document.EntityId}`);if(flow?.DocumentInfo?.IsDeleted)return null;let xml="";try{xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,document.EntityId);}catch{}const isForwarding=document.TypeNamedId==="LogisticsForwardingOrder",isOrder=document.TypeNamedId==="LogisticsOrderRequest";const firstParty=(...tags)=>tags.map(tag=>xmlParty(xml,tag)).find(value=>value!=="Не указано")||"Не указано";const client=isForwarding?firstParty("СвКл","СвЗакТЭУ","СвЗак"):firstParty("СвЗак","СвГО");const carrier=isForwarding?firstParty("СвЭксп","СвИсп","СвПер"):firstParty("СвПер","СвИсп");const titles=(flow?.Docflow?.Titles||[]).filter(title=>title.AuthorSigning);const signed=Boolean(titles.length)&&titles.every(title=>title.AuthorSigning?.IsFinished===true);const rawDate=document.DocumentDate||document.CreationTimestamp||"";const match=String(rawDate).match(/^(\d{2})\.(\d{2})\.(\d{4})/);const parsed=match?new Date(`${match[3]}-${match[2]}-${match[1]}T00:00:00+03:00`):new Date(rawDate);const month=Number.isNaN(parsed.getTime())?"Не определён":`${parsed.getFullYear()}-${String(parsed.getMonth()+1).padStart(2,"0")}`;return {month,documentType:isForwarding?"Поручение":isOrder?"Заявка":"ЭТрН",client,carrier,signed};});
  const value={generatedAt:new Date().toISOString(),fromMonth:"2026-09",items:items.filter(Boolean)};statisticsCache.set(cacheKey,{expiresAt:Date.now()+10*60_000,value});return value;
}
async function loadSigningControl(){
  ...
  const items=(await mapLimit(unique,5,async document=>{
    ...
    const isOrder=document.TypeNamedId==="LogisticsOrderRequest";const container=(xml.match(/[A-Z]{4}\d{7}/)||[])[0]||"—";const firstParty=(...tags)=>tags.map(tag=>xmlParty(xml,tag)).find(value=>value!=="Не указано")||"Не указано";const client=isForwarding?firstParty("СвКл","СвЗакТЭУ","СвЗак"):xmlParty(xml,"СвЗак");const carrier=isForwarding?firstParty("СвЭксп","СвИсп","СвПер"):(xmlParty(xml,"СвПер")!=="Не указано"?xmlParty(xml,"СвПер"):xmlParty(xml,"СвИсп"));const consignee=xmlParty(xml,"СвГП");
    const trackedTitles=forwardingAgreed?[{TitleIndex:1,agreed:true}]:waiting;const serviceCost=isForwarding?forwardingServiceCost(xml):"";const forwardingDirection=isForwarding&&/ТАГЛЕКС/i.test(consignee)?"incoming":"outgoing";
    return trackedTitles.map(title=>{const titleNumber=Number(title.TitleIndex||0)+1;let controlDate=null;let controlDateLabel="Дата документа";
      if(isForwarding){controlDate=addHours(documentDate(document.DocumentDate)||new Date(document.CreationTimestamp||Date.now()),Number(process.env.KONTUR_FORWARDING_RESPONSE_HOURS||24));controlDateLabel=forwardingDirection==="incoming"?"Срок нашей подписи":"Срок подписи экспедитора";}
      ...
      const agreed=Boolean(title.agreed);const responsible=isForwarding?(forwardingDirection==="incoming"?"Мы":"Экспедитор"):titleNumber===3?"Грузополучатель":titleNumber===1?"Грузоотправитель":"Перевозчик";
      return {id:`${document.MessageId}:${document.EntityId}:${titleNumber}`,documentType:isForwarding?"Поручение экспедитору":isOrder?"Заявка":"ЭТрН",workflowGroup:isForwarding?forwardingDirection:null,direction:isForwarding?forwardingDirection:null,agreed,serviceCost,number:document.DocumentNumber||xmlAttribute(xml,isForwarding?"НомДок":isOrder?"НомерЗаяв":"НомерТрН")||"Без номера",container,client,carrier,consignee,waitingTitle:agreed?"Согласовано":`Т${titleNumber}`,responsible,status:agreed?"agreed":delay.status,statusText:agreed?"Подписано экспедитором":isForwarding&&forwardingDirection==="incoming"?"Требуется наша подпись":`Ожидается подпись: ${responsible.toLowerCase()}`,controlDate:controlDate.toISOString(),controlDateLabel,overdueText:agreed?"Согласовано":delay.text,messageId:document.MessageId,entityId:document.EntityId,sourceMessageId,sourceEntityId,documentUrl:konturDocumentUrl(config.boxId,document.MessageId,customData["kl-id"]||sourceEntityId,document.TypeNamedId)};});
  })).flat();
...
const server = http.createServer((request, response) => {
  ...
  if(request.method==="GET"&&url.pathname==="/api/kontur/statistics"){
    void(async()=>{try{const result=await loadDocumentStatistics();response.writeHead(200,{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"});response.end(JSON.stringify(result));}catch(error){response.writeHead(502,{"Content-Type":"application/json; charset=utf-8"});response.end(JSON.stringify({error:error.message||"Не удалось получить статистику"}));}})();return;
  }
  ...
  if(request.method==="GET"&&url.pathname==="/api/kontur/powers-of-attorney"){
    void(async()=>{try{const config=konturConfig();const accessToken=await getKonturAccessToken();const result=await diadocJson(`/GetEmployeePowersOfAttorney?boxId=${encodeURIComponent(config.boxId)}&onlyActual=true`,accessToken);const items=(result.PowersOfAttorney||[]).map(item=>{const value=item.PowerOfAttorney||{};const issuer=value.Issuer?.LegalEntity||value.Issuer?.IndividualEntity||value.Issuer?.ForeignEntity||{};const person=value.Confidant?.PersonName||{};return {fullId:value.FullId,isDefault:Boolean(item.IsDefault),issuerName:issuer.OrganizationName||"Доверитель не указан",representativeName:[person.LastName,person.FirstName,person.MiddleName].filter(Boolean).join(" ")||"Представитель не указан",validFrom:value.StartAtByMoscow||value.StartAt||null,validTo:value.ExpireAtByMoscow||value.ExpireAt||null};}).filter(item=>item.fullId?.RegistrationNumber&&item.fullId?.IssuerInn&&item.fullId?.RepresentativeInn);response.writeHead(200,{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"});response.end(JSON.stringify({items}));}catch(error){response.writeHead(400,{"Content-Type":"application/json; charset=utf-8"});response.end(JSON.stringify({error:error.message||"Не удалось получить МЧД"}));}})();return;
  }
  ...
  if(request.method==="POST"&&url.pathname==="/api/kontur/forwarding-revoke/send"){
    const chunks=[];request.on("data",chunk=>chunks.push(chunk));request.on("end",()=>void(async()=>{try{
      const payload=JSON.parse(Buffer.concat(chunks).toString("utf8"));for(const key of ["messageId","parentEntityId","contentBase64","signatureBase64"])if(!String(payload[key]||"").trim())throw new Error(`Не заполнено поле: ${key}`);const content=Buffer.from(payload.contentBase64,"base64");const signature=Buffer.from(payload.signatureBase64,"base64");if(!content.length||content.length>2_000_000)throw new Error("Некорректный размер XML");if(!signature.length||signature.length>1_000_000)throw new Error("Некорректный размер подписи");const xml=content.toString("utf8");if(!xml.includes("СодИнфОтз")||!xml.includes("ПричОтз="))throw new Error("Передан не титул отзыва поручения");const config=konturConfig();const accessToken=await getKonturAccessToken();const flowResult=await diadocJson(`/V5/GetDocflows?boxId=${encodeURIComponent(config.boxId)}`,accessToken,{method:"POST",body:JSON.stringify({Requests:[{DocumentId:{MessageId:payload.messageId,EntityId:payload.parentEntityId}}]})});const flow=flowResult.Documents?.[0];const typeNamedId=flow?.DocumentInfo?.DocumentType?.TypeNamedId;if(!flow||typeNamedId&&typeNamedId!=="LogisticsForwardingOrder")throw new Error("Исходное поручение не найдено");const titles=flow.Docflow?.Titles||[];if(!titles.some(title=>Number(title.TitleIndex)===1&&title.AuthorSigning?.IsFinished===true))throw new Error("Т2 экспедитора ещё не подписан");if(titles.some(title=>Number(title.TitleIndex)===2))throw new Error("Т3 уже отправлен");const fullId=payload.powerOfAttorney?.fullId;const powerOfAttorney=payload.powerOfAttorney?.useDefault===true?{UseDefault:true}:fullId&&/^[0-9a-f-]{36}$/i.test(String(fullId.RegistrationNumber||""))&&/^\d{10,12}$/.test(String(fullId.IssuerInn||""))&&/^\d{10,12}$/.test(String(fullId.RepresentativeInn||""))?{FullId:{RegistrationNumber:fullId.RegistrationNumber,IssuerInn:fullId.IssuerInn,RepresentativeInn:fullId.RepresentativeInn},UseDefault:false}:null;const patch=await diadocJson("/V4/PostMessagePatch",accessToken,{method:"POST",body:JSON.stringify({BoxId:config.boxId,MessageId:payload.messageId,RecipientTitles:[{ParentEntityId:payload.parentEntityId,TitleIndex:2,SignedContent:{Content:payload.contentBase64,Signature:payload.signatureBase64,...(powerOfAttorney?{PowerOfAttorney:powerOfAttorney}:{})}}]})});signingControlCache.value=null;response.writeHead(200,{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"});response.end(JSON.stringify({ok:true,messageId:patch.MessageId,patchId:patch.PatchId}));
